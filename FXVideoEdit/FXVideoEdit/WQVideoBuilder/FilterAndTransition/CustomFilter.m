...
- (CVPixelBufferRef)renderByCIImage:(CVPixelBufferRef)pixelBuffer {
    ...
    image = [image imageByApplyingFilter:@"CIPhotoEffectFade"];

    CGSize defaultSize = CGSizeMake(960, 540);
    CGSize natureSize = CGSizeMake(540, 960);

    CGFloat scale = MIN(defaultSize.width / natureSize.width, defaultSize.height / natureSize.height);
    // 移至中心点
    CGPoint translate = CGPointMake((defaultSize.width - natureSize.width * scale) * 0.5, (defaultSize.height - natureSize.height * scale) * 0.5);
    CGAffineTransform naturalTransform = CGAffineTransformIdentity;
