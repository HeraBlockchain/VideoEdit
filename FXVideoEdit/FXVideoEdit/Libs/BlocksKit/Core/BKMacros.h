...
#import "NSSet+BlocksKit.h"
    ...
    #define BK_EACH(collection, ...) __BK_EACH_WRAPPER_VOID([collection bk_each:^(id obj){ \
        __VA_ARGS__}])
    #define BK_MAP(collection, ...) __BK_EACH_WRAPPER([collection bk_map:^id(id obj) { \
        return (__VA_ARGS__);                                                          \
    }])
    #define BK_SELECT(collection, ...) __BK_EACH_WRAPPER([collection bk_select:^BOOL(id obj) { \
        return (__VA_ARGS__) != 0;                                                             \
    }])
    #define BK_REJECT(collection, ...) __BK_EACH_WRAPPER([collection bk_select:^BOOL(id obj) { \
        return (__VA_ARGS__) == 0;                                                             \
    }])
    #define BK_MATCH(collection, ...) __BK_EACH_WRAPPER([collection bk_match:^BOOL(id obj) { \
        return (__VA_ARGS__) != 0;                                                           \
    }])
    #define BK_REDUCE(collection, initial, ...) __BK_EACH_WRAPPER([collection bk_reduce:(initial)         \
                                                                              withBlock:^id(id a, id b) { \
                                                                                  return (__VA_ARGS__);   \
                                                                              }])
    ...
    #define BK_APPLY(collection, ...) [collection bk_apply:^(id obj){ \
        __VA_ARGS__}]
