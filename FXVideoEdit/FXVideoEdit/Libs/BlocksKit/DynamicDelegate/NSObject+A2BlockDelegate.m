...
static inline BOOL isValidIMP(IMP impl) {
    ...
    if (impl == NULL || impl == _objc_msgForward)
        return NO;
#else
    if (impl == NULL || impl == _objc_msgForward || impl == (IMP)_objc_msgForward_stret)
        return NO;
#endif
    return YES;
}
...
+ (NSMapTable *)bk_delegateInfoByProtocol:(BOOL)createIfNeeded {
    ...
    if (delegateInfo || !createIfNeeded) {
        return delegateInfo;
    }
